...
<x-layouts.app
    :page="$page"
    title="Contact us — CoreX OS"
    description="Get in touch with the CoreX OS team — questions, pricing, support, partnerships or anything else. We reply within one business day."
>
            ...
            <div class="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
                ...
                <div>
                    <x-section-heading
                        ...
                        eyebrow-icon="mail"
                        align="left"
                        title='Talk to the <span class="text-gradient">CoreX OS team.</span>'
                    >
                        A question about the platform, pricing, help on an existing account, or something else
                        entirely &mdash; send it here and a person will come back to you.
                    </x-section-heading>

                    <ul class="mt-8 space-y-3">
                        @foreach ([
                            'A reply from a real person within one business day',
                            'Existing customers: tell us your agency and we route it to support',
                            'Not sure yet? Ask anything — no sales pitch unless you want one',
                        ] as $point)
                    ...
                    <div class="reveal mt-8 card divide-y divide-[color:var(--color-border-soft)]">
                        <a href="mailto:{{ $contactEmail }}" class="flex items-center gap-4 p-5 transition duration-300 hover:bg-[color:var(--color-surface-2)]">
                            <span class="grid h-10 w-10 shrink-0 place-items-center rounded-md border border-[color:var(--color-border)] text-[color:var(--color-brand)]">
                                <x-icon name="mail" class="w-5 h-5" />
                            </span>
                            <span class="min-w-0">
                                <span class="block text-sm font-medium text-ink">Email us directly</span>
                                <span class="block truncate text-sm text-[color:var(--color-muted)]">{{ $contactEmail }}</span>
                            </span>
                        </a>
                        <a href="{{ route('home') }}#demo" class="flex items-center gap-4 p-5 transition duration-300 hover:bg-[color:var(--color-surface-2)]">
                            <span class="grid h-10 w-10 shrink-0 place-items-center rounded-md border border-[color:var(--color-border)] text-[color:var(--color-brand)]">
                                <x-icon name="presentation" class="w-5 h-5" />
                            </span>
                            <span class="min-w-0">
                                <span class="block text-sm font-medium text-ink">Prefer a walkthrough?</span>
                                <span class="block text-sm text-[color:var(--color-muted)]">Book a 30-minute demo on your own deals instead.</span>
                            </span>
                        </a>
                    </div>
                </div>
                ...
                <div class="reveal">
                    <div class="card p-6 sm:p-8 shadow-2xl shadow-black/30">
                        @if (session('contact_success'))
                            ...
                            <x-conversion-event form="contact" />
                        @else
                            <form
                                method="POST"
                            ...
                            >
                                ...
                                <div class="grid gap-4 sm:grid-cols-2">
                                    ...
                                    <div>
                                        <label for="agency" class="mb-1.5 block text-sm font-medium text-ink">Agency <span class="text-[color:var(--color-faint)]">(optional)</span></label>
                                        <input id="agency" name="agency" type="text" value="{{ old('agency') }}" autocomplete="organization"
                                ...
                                <div>
                                    <label for="topic" class="mb-1.5 block text-sm font-medium text-ink">What is this about?</label>
                                    <select id="topic" name="topic"
                                            class="{{ $fieldBase }} @error('topic') border-[#e11d48] @else border-[color:var(--color-border)] @enderror">
                                        <option value="" @selected(old('topic') === null || old('topic') === '')>Choose one (optional)</option>
                                        @foreach ($topics as $value => $label)
                                            <option value="{{ $value }}" @selected(old('topic') === $value)>{{ $label }}</option>
                                        @endforeach
                                    </select>
                                    @error('topic') <p class="mt-1.5 text-xs text-[#fb7185]">{{ $message }}</p> @enderror
                                </div>
                                ...
                                <div>
                                    <label class="flex items-start gap-3 text-sm text-[color:var(--color-muted)]">
                                        ...
                                        <span>I agree to CoreX OS contacting me about this message. We&rsquo;ll only use your details for that — in line with POPIA.</span>
                                    </label>
